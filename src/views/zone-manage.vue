<template>
  <div>
    ...
    <van-tabs v-model:active="activeTab">
      ...
      <!-- 区域管理 Tab -->
      <van-tab title="区域管理">
        <van-button type="primary" size="small" icon="plus" style="margin:12px 16px" @click="openZoneForm(null)">
          添加区域
        </van-button>
        <van-empty v-if="zones.length === 0" description="暂无区域" />
        <van-swipe-cell v-for="zone in zones" :key="zone.id">
          <van-cell @click="openZoneForm(zone)">
            <template #title>
              <span class="color-dot" :style="{ background: zone.color || '#ccc' }" />
              {{ zone.name }}
            </template>
            <template #label>编码：{{ zone.code }}</template>
          </van-cell>
          <template #right>
            <van-button square type="danger" text="删除" style="height:100%" @click="confirmDeleteZone(zone)" />
          </template>
        </van-swipe-cell>
      </van-tab>
      ...
      <van-tab title="库位管理">
        <div style="padding:12px 16px">
          <van-field v-model="batchZoneCode" label="区域编码" placeholder="如 A" style="margin-bottom:8px" />
          <van-field v-model="batchCount" label="数量" type="number" placeholder="如 10" />
          <van-button type="primary" block style="margin-top:8px" @click="batchAddSlots">批量添加库位</van-button>
        </div>
        ...
        <template v-for="zone in zonesWithSlots" :key="zone.code">
          <van-cell-group :title="zone.name">
            <van-cell v-for="slot in zone.slots" :key="slot.id" :title="slot.slot_id">
              <template #right-icon>
    ...
    <!-- 区域表单弹窗 -->
    <van-popup v-model:show="zoneFormShow" position="bottom" round :style="{ height: '70%' }">
      <div style="padding:16px">
        ...
        <van-field v-model.number="zoneForm.w" label="宽" type="number" placeholder="200" />
        <van-field v-model.number="zoneForm.h" label="高" type="number" placeholder="100" />
        <van-button type="primary" block style="margin-top:12px" @click="submitZoneForm">保存</van-button>
      </div>
    </van-popup>
  </div>
</template>
...
const zoneForm = ref({ name: '', code: '', color: '', x: 0, y: 0, w: 200, h: 100 })
...
const zonesWithSlots = computed(() =>
  zones.value.map(z => ({
    ...z,
    slots: slots.value.filter(s => s.zone_code === z.code),
  })).filter(z => z.slots.length > 0)
)

async function loadData() {
  const [zRes, sRes] = await Promise.all([getList('zones'), getList('slots')])
...
function openZoneForm(zone) {
  ...
  zoneForm.value = zone
    ...
    : { name: '', code: '', color: '#1989fa', x: 0, y: 0, w: 200, h: 100 }
...
async function submitZoneForm() {
  ...
  if (editingZone.value) {
    await updateDoc('zones', editingZone.value.id, zoneForm.value)
  } else {
...
function confirmDeleteZone(zone) {
  showConfirmDialog({ title: '确认删除', message: `删除区域「${zone.name}」？` })
    .then(async () => { await removeDoc('zones', zone.id); await loadData() })
    .catch(() => {})
}
...
async function batchAddSlots() {
  ...
  if (!code || !count || count <= 0) { showToast('请填写区域编码和数量'); return }
  const tasks = []
  for (let i = 1; i <= count; i++) {
    tasks.push(addDoc('slots', { slot_id: `${code}-${String(i).padStart(2, '0')}`, zone_code: code, x: 0, y: 0, status: 'empty' }))
  }
...
.form-title { font-size: 16px; font-weight: bold; margin-bottom: 12px; }
</style>
